...
The t-value ($t_{0.95}$) with `r testlenbysupp$parameter` degree of freedom is `r round(qt(0.95,testlenbysupp$parameter),2)`. The $t_{0.95}$ value iss les than the test statistics so we can reject the null hypothesis.  
...
`r pander(res[c(1,2,3)])`  

The t-value ($t_{0_95}$) with `r res0.5$df` degree of freedom is `r res0.5$parameter`. The $t_{0.95}$ value is lower than the test statistics so we can reject the null hypothesis. There's a difference in mean for toothength by mean of administration at the 0.5 mg dose level.  
